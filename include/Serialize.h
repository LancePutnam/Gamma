...
Byte ordering is little endian. If the target architecture is big endian, then pass in
the preprocessor flag -DSER_IS_BIG_ENDIAN.
...
inline const char * serTypeAsString(uint8_t t){
	switch(t){
...
inline int serTypeSize(uint8_t t){
	switch(t){
...
inline const char * serHeaderAsString(const SerHeader * h){
	static char s[32];
	sprintf(s, "%s %d", serTypeAsString(h->type), h->num);
	return s;
}

inline uint32_t serElementsSize(const SerHeader * h){
...
uint32_t serEncodeFloat32(char * b, const float * v   , uint32_t n){ return serEncode4(SER_FLOAT32, b,v,n); }
uint32_t serEncodeFloat64(char * b, const double * v  , uint32_t n){ return serEncode8(SER_FLOAT64, b,v,n); }
uint32_t serEncodeInt8   (char * b, const int8_t * v  , uint32_t n){ return serEncode1(SER_INT8   , b,v,n); }
uint32_t serEncodeInt16  (char * b, const int16_t * v , uint32_t n){ return serEncode2(SER_INT16  , b,v,n); }
uint32_t serEncodeInt32  (char * b, const int32_t * v , uint32_t n){ return serEncode4(SER_INT32  , b,v,n); }
uint32_t serEncodeInt64  (char * b, const int64_t * v , uint32_t n){ return serEncode8(SER_INT64  , b,v,n); }
uint32_t serEncodeBool   (char * b, const bool * v    , uint32_t n){ return serEncode1(SER_UINT8  , b,v,n); }
uint32_t serEncodeUInt8  (char * b, const uint8_t * v , uint32_t n){ return serEncode1(SER_UINT8  , b,v,n); }
uint32_t serEncodeUInt16 (char * b, const uint16_t * v, uint32_t n){ return serEncode2(SER_UINT16 , b,v,n); }
uint32_t serEncodeUInt32 (char * b, const uint32_t * v, uint32_t n){ return serEncode4(SER_UINT32 , b,v,n); }
uint32_t serEncodeUInt64 (char * b, const uint64_t * v, uint32_t n){ return serEncode8(SER_UINT64 , b,v,n); }
...
template<class T> uint32_t encode(char * b, const T * v, uint32_t n){ return 0; }

#define DEF(t,T,c)\
template<> uint32_t encode(char * b, const t * v, uint32_t n){ return serEncode##T(b, c v,n); }
DEF(float,   Float32,)
DEF(double,  Float64,)
...
struct Serializer{
	...
	const std::vector<char>& buf() const { return mBuf; }
...
struct Deserializer{
	...
	Deserializer(const std::vector<char>& b): mBuf(b), mStart(0){}
...
private:
	...
	char * bufDec(){ return &mBuf[mStart]; }
};
